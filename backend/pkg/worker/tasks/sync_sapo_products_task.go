...
func NewSyncSapoProductsRecursiveTask(page, limit int, store string) (*asynq.Task, error) {
	payload, err := json.Marshal(SyncSapoProductsPayload{Page: page, Limit: limit, Store: store})
	if err != nil {
		return nil, err
	}
...
func HandleSyncSapoProductsRecursiveTask(ctx context.Context, t *asynq.Task) error {
	...
	count, err := sapo.SyncProducts(p.Page, p.Limit, p.Store)
	if err != nil {
		return err
	}

	if count > 0 {
		payload, err := json.Marshal(SyncSapoProductsPayload{Page: p.Page + 1, Limit: p.Limit, Store: p.Store})
		if err != nil {
			return err
		}
